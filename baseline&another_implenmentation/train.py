...
def train(arg, model, train_set, test_set, device,testdataset=None, writer=None,print_flag=True,fold_id='0'):
    ...
    for epoch in range(arg.num_epochs):
        ...
        if writer != None:
            ...
            writer.add_scalars(fold_id+'scalar/ce_loss', {'train_loss': r1['loss'], 'val_loss': r2['loss']}, epoch)
...
def cross_val(arg,feature_dim,train_graphs,val_graphs,device,writer=None,fold_id=0):
    ...
    if arg.method == 'others':
       print("tree method")
        ...
        print("please used baseline_train.py !")
    ...
    print("creat Model!!!")
    model = THAN.TreeGCN(feature_dim,attention=arg.attention, num_layers=2, hidden_dim=arg.hidden_dim, class_dim=arg.num_classes, pre_hidden_dim=[128,32], pooling_type=arg.method,\
...
if __name__=="__main__":
    ...
    if k<10:
        ...
        writer = SummaryWriter(log_dir=writer_path)
        train_acc, val_acc = cross_val(args, feature_dim, train_graphs, val_graphs, device, writer = writer, fold_id=k)
        writer.close()
